...
namespace SterlingWrapper
{
    public class Connector
    {
        ...
        public string Sendoptionlimit(string side, string symbol, string account, string tif, int quantity, string destination, double lmt_price, string open_close, string maturity, string put_call, string underlying, string cover_uncover, string instrument, double strike)
        {
            ...
            stiOrder.Destination = destination;
            stiOrder.LmtPrice = lmt_price;

            // option chet
            stiOrder.OpenClose = open_close; // O for open, C for close
            stiOrder.Maturity = maturity; // YYYYMMDD
            stiOrder.PutCall = put_call; // P or C
            stiOrder.Underlying = underlying; // SPY
            stiOrder.CoverUncover = cover_uncover; // C for cover, U for uncover
            stiOrder.Instrument = instrument; // O for options, E for equities
            stiOrder.StrikePrice = strike;
            ...
            string data = String.Format("{0}, {1} {2} {3} {4} status: {5} ordID: {6}", DateTime.Now.ToLongTimeString(), symbol, lmt_price, side, destination, status, ord_id);
            WriteLog(data);
        ...
        public void CancelOptionsOrder(string account, string order_id)
        {
            string new_id = order_id + "cancel";
            stOrdMaint.CancelOptionsOrder(account, 0, order_id, new_id);
            int st = OrderStatus(order_id);
            Console.WriteLine("Sterling API ~ Cancel status..." + st + " for order id: " + order_id);
        }
        ...
        public int FillQty(string order_id)
        {
            return stOrdMaint_req.GetOrderInfo(order_id).nCumExecQuantity;
        }

        public double FillPrice(string order_id)
        {
            return stOrdMaint_req.GetOrderInfo(order_id).fAvgExecPrice;
        }
